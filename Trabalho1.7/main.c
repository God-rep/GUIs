...
int main(int argc, char* args[]){
	...
	SDL_Texture* img = IMG_LoadTexture(ren, "Transparent Donkey KNEW.png");	// Importa img

	assert(img != NULL);	// Assegurar que img foi importada
	...
	*/
	...
	while(1){
		...
		if(isevt){	// Se houver evento nao-temporal no intervalo
			...
			switch(evt.type){
				...
				case SDL_KEYDOWN:
					...
					switch(evt.key.keysym.sym){
						...
						/*	Forma antiga de detectar posicao do jogador
						 *	Antes, jogador era uma bola com centro sendo representado por r
						case SDLK_RIGHT:
						...
						*/
						...
						// Correcao de detecao de movimento do jogador para imagem
						case SDLK_RIGHT:
                                                        SDL_GetWindowSize(win, &xWin, &yWin);

                                                        if((r.x+r.w+5)< xWin){     //Centro do circulo + raio + deslocamento
                                                                r.x+=5;
                                                        }
                                                        else{
                                                                r.x = xWin-r.w; //Circulo na extremidade direita
                                                        }
                                                        break;
                                                case SDLK_LEFT:
                                                        if(r.x > 5){     //Centro do circulo + raio + deslocamento
                                                                r.x-=5;
                                                        }
                                                        else{
                                                                r.x = 0;       //Circulo na extremidade esquerda
                                                        }
                                                        break;
			...
			/*	Gravidade tambem usa posicao do jogador
			 *	Logo, tambem deve ser modificada para adequar-se a imagem
